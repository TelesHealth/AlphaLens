...
- **Outcome Resolution (E10)**: Automated daily 9:00 AM UTC cron resolves open `trade` recommendations against truth sources — Kalshi finalized markets, Polymarket closed markets, current asset prices for crypto/equities/FX/commodities, and macro releases (CPI, GDP, unemployment, fed funds) for macro recs. Per-process Kalshi cache eliminates 429 storms. Recommendations are tagged `resolutionMethod: "auto" | "manual" | null`; admin PATCH writes "manual". Admin-only `POST /api/recommendations/resolve-outcomes` triggers an immediate run and returns a digest (resolved today, still open, approaching window 7d, needs human review). Public leaderboard exposes `autoResolved`, `manualResolved`, and `pendingResolution` (overdue trade calls, derived from the same shared categorizer the digest uses).
